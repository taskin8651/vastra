@php
    $pageSlug = isset($audience) ? $audience->slug : 'category';

    $pageTitle = isset($audience)
        ? $audience->name . ' - Vastra Express'
        : 'Vastra Express';
@endphp

<!doctype html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="theme-color" content="#fff">

    <title>@yield('title', $pageTitle)</title>

    <link rel="preconnect" href="https://cdn.jsdelivr.net">

    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css" rel="stylesheet">

    <link rel="stylesheet" href="{{ asset('assets/css/style.css') }}">
</head>

<body class="category-page reference-{{ $pageSlug }}-page">

    <div class="site-wrap">

        {{-- PHONE STATUS --}}
        <div class="phone-status">
            <span>8:00</span>

            <span class="phone-status-icons">
                <i class="bi bi-reception-4"></i>
                <i class="bi bi-wifi"></i>
                <i class="bi bi-battery-full"></i>
            </span>
        </div>

        {{-- HEADER --}}
        <header class="category-header">

            <a href="{{ url('/') }}" class="detail-icon">
                <i class="bi bi-list"></i>
            </a>

            <a href="{{ url('/') }}" class="brand">
                <span class="brand-mark">
                    <span>V</span>
                </span>

                <span class="brand-name">
                    VASTRA<span>EXPRESS</span>
                </span>
            </a>

            <div class="header-actions">

                <a class="icon-btn" href="{{ route('frontend.search') }}" aria-label="Search">
                    <i class="bi bi-search"></i>
                </a>

                <a class="icon-btn" href="#">
                    <i class="bi bi-person"></i>
                </a>

                <a class="icon-btn cart-button" href="{{ url('/cart') }}">
                    <i class="bi bi-bag"></i>
                </a>

            </div>

        </header>

        {{-- PAGE CONTENT --}}
        @yield('content')

        {{-- BOTTOM NAV --}}
        <nav class="bottom-nav">

            <a href="{{ url('/') }}">
                <i class="bi bi-house-door-fill"></i>
                <span>Home</span>
            </a>

            <a class="active" href="{{ isset($audience) ? route('frontend.audience.show', $audience) : url('/') }}">
                <i class="bi bi-grid-3x3-gap-fill"></i>
                <span>Categories</span>
            </a>

          <a href="{{ route('frontend.wishlist.index') }}">
    <i class="bi bi-heart"></i>
    <span>Wishlist</span>
</a>

            <a href="#">
                <i class="bi bi-person"></i>
                <span>Account</span>
            </a>

            <a href="{{ url('/cart') }}">
                <i class="bi bi-bag"></i>
                <span>Orders</span>
            </a>

        </nav>

    </div>

    <div class="toast-container position-fixed end-0 p-3">
        <div id="cartToast" class="toast align-items-center text-bg-dark border-0" role="status" aria-live="polite" aria-atomic="true">
            <div class="d-flex">
                <div class="toast-body">Product added to cart.</div>
                <button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast" aria-label="Close"></button>
            </div>
        </div>
    </div>

    <script src="{{ asset('assets/js/main.js') }}"></script>
</body>

</html>
