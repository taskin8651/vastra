...
class CategoriesController extends Controller
{
    ...
    public function store(Request $request) { Category::create($this->validated($request)); return redirect()->route('admin.categories.index')->with('message', 'Category created.'); }
    ...
    public function update(Request $request, Category $category) { $category->update($this->validated($request, $category)); return redirect()->route('admin.categories.index')->with('message', 'Category updated.'); }
    public function destroy(Category $category) { $category->delete(); return back()->with('message', 'Category deleted.'); }
    private function validated(Request $request, ?Category $category = null): array
    {
        $data = $request->validate(['audience_id' => 'required|exists:audiences,id', 'name' => 'required|string|max:100', 'slug' => 'nullable|alpha_dash|max:100|unique:categories,slug,'.optional($category)->id, 'image_path' => 'nullable|string|max:255', 'sort_order' => 'nullable|integer|min:0']);
